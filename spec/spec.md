...
画面の上部にクイズの対象とする楽曲の制限をタグ選択形式で絞り込むようにしましょう。ジャンル、投稿された年代の幅、言語などの条件を扱います。ジャンルは属性であり、一つの楽曲が複数の属性ラベルを持つことができる。
...
* **リザルト(result)画面**: 正解またはスキップしたらここに到達。(spec/game_images/03_result_draft.JPG)ここでは楽曲の情報を全て公開する。さらに動画や楽曲へのリンクも表示。次の問題をサジェストする。押されたらクイズプレイ画面へ。
* **プレイ情報(library)画面**: コメントや歌詞、動画、アーティストという各粒度でお気に入り登録/確認が可能。ここに表示するのはYouTube APIを叩く必要のない程度に抑えましょう。タブで粒度を切り替えてその一覧を表示し、紐着く動画(楽曲タイトル)を表示。詳細を見たい時には楽曲詳細情報としてresult画面に似たものを表示(spec/game_images/04_library_music_draft.JPG)。ここで初めてAPIを叩きましょう。
