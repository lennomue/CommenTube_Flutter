...
4. OpenAIまたはJev: ルール通過候補を動画固有性、有用性、答え漏れ、ノイズ、言語の偏りで評価し、人が確認する短いリストを最大15件程度にする。
5. Google Sheets: 15件程度の候補を1コメント1行で出力し、`reviewer_selected`と`reviewer_notes`を人が編集する。
6. 最終採用: 人が1〜8件を選ぶ。`thumbnail_hint_type=comment`なら、並び順の先頭をHomeとGameで最初から見える代表コメントにする。残りはGameで開放するヒントになる。
7. 表示タイトル、ジャンル、言語、アーティスト候補等も人が確認し、承認済みデータだけを将来の投入コマンドでSupabaseへ追加する。

現在の`draft-openai`は最大5件を仮採用する実装で、最大15件のSheetsレビュー候補と最終1〜8件の確定を分離する処理は未実装です。Sheets同期と一緒に、候補・最終採用を別フィールドとして実装します。それまでは生成JSONをSupabase投入可能な確定データとは扱いません。
...
| シート | 1行の単位 | 用途 |
|---|---|---|
| `comment_candidates` | コメント候補1件 | 最大15件のAI/Jev評価、人の採否・メモ・最終順序 |
| `quiz_drafts` | 動画1件 | 表示タイトル、ジャンル、言語、代表ヒント、承認状態 |
| `artists` | アーティスト候補1件 | 既存IDとの同一性、代表名、別名、人の確認 |
| `relations` | 関連候補1件 | 既存動画・アーティストとの関係を人が確定 |

同期は行の単純追記だけにせず、`video_id + comment_id`等の安定キーで同じ候補行を更新します。Pythonから再出力しても`reviewer_selected`、`reviewer_notes`、`final_order`を上書きしない設計にします。Supabaseへの投入はSheetsから自動連鎖させず、承認済み行を読み戻して検証する別コマンドにします。
